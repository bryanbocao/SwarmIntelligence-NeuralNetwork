	...
	public BPNeuralNetwork(){
		...
		for (int i = 0; i < inputLayerSize; i++) {
			for (int j = 0; j < hiddenLayerSize; j++) {
				//initialize weights from -10 to 10 
				weightsL0toL1[i][j] = Math.random()*2 - 1;
				//System.out.println(weightsL0toL1[i][j]);
			}
		}
		
		for (int i = 0; i < hiddenLayerSize; i++) {
			//initialize weights from -10 to 10 
			weightsL1toL2[i] = Math.random()*20 - 10;
//			System.out.println(weightsL1toL2[i]);
		}
...
//		System.out.println(this.deltaL2);
		...
		for (int i = 0; i < hiddenLayerSize; i++) {
			deltaL1[i] = outputL1[i] * (1 - outputL1[i]) * deltaL2 * weightsL1toL2[i];
		}
